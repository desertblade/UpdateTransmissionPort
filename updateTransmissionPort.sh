#! /bin/sh
#Need to add path so it will work with cron
...
#find the ip to the host
local_vpn_ip=$(ifconfig tun0 | grep inet | awk '{ print $2 }')
pia_client_id_file=/usr/local/etc/openvpn/pia_client_id
...
new_client_id() {
    head -n 100 /dev/urandom | md5 | tr -d " -" | tee $pia_client_id_file
}
...
# Get the port
pia_response=$(curl -k -d "user=$pia_username&pass=$pia_passwd&client_id=$pia_client_id&local_ip=$local_vpn_ip" $port_assignment_url)
